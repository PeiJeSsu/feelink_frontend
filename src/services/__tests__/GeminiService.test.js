	...
	describe("generateImage", () => {
		...
		test("應正確處理純文字提示", async () => {
			const mockResponse = {
				response: {
					candidates: [
						{
							content: {
								parts: [{ text: "生成的描述" }, { inlineData: { data: "generated-image-data" } }],
							},
						},
					],
				},
			...
			expect(mockGenerateContent).toHaveBeenCalledWith(
				expect.arrayContaining([expect.objectContaining({ text: expect.stringContaining(mockPrompt) })])
			);
			...
			const mockResponse = {
				response: {
					candidates: [
						{
							content: {
								parts: [{ text: "圖片描述" }, { inlineData: { data: "generated-image-data" } }],
							},
						},
					],
				},
			...
			const mockResponse = {
				response: {
					candidates: [
						{
							content: {
								parts: [{ inlineData: { data: "only-image-data" } }],
							},
						},
					],
				},
		...
		test("應在生成失敗時拋出錯誤", async () => {
			...
			await expect(geminiService.generateImage(mockPrompt)).rejects.toThrow("API 錯誤");
		});
	});
});
