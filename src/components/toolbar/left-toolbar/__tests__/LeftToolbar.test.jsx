  ...
  const MockSettingsPopover = ({ open, children, title, onClose }) => (
    open ? (
      <div data-testid={`popover-${title}`}>
        <h3>{title}</h3>
        <button onClick={onClose} data-testid={`close-${title}`}>關閉</button>
        {children}
      </div>
    ) : null
  );
...
jest.mock("@mui/material", () => ({
  Paper: function Paper(props) {
    return <div className={props.className} data-testid="paper">{props.children}</div>;
  }
}));
