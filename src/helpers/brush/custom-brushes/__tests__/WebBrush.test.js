...
describe("WebBrush", () => {
	...
	it("建構時應正確設置屬性", () => {
		const brush = new WebBrush(mockCanvas, { color: "#123", opacity: 0.5, width: 10 });
	...
	it("onMouseDown 應初始化 points 並設置 colorValues", () => {
		const brush = new WebBrush(mockCanvas, { color: "#123" });
		const pointer = { x: 1, y: 2 };
		brush.onMouseDown(pointer);
		expect(brush._points).toEqual([pointer]);
		expect(brush._count).toBe(0);
		expect(brush._colorValues).toEqual([1, 2, 3]);
	});

	it("onMouseMove 應繪製主線與隨機線", () => {
		const brush = new WebBrush(mockCanvas, { width: 5 });
		brush._points = [
			{ x: 0, y: 0 },
			{ x: 1, y: 1 },
		];
		brush._count = 0;
		brush._colorValues = [1, 2, 3];
